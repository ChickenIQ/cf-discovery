...
export const validateEntry = async (masterKey: string, e: Entry): Promise<string | null> => {
  const now = Date.now();
  ...
  const memberErr = await verifySignature(masterKey, e.member.signature, memberData);
  if (memberErr) return `Invalid memberSignature: ${memberErr}`;

  const bodyData = e.member.metadata.toString() + e.body.data.toString() + e.body.timestamp.toString();
  const bodyErr = await verifySignature(masterKey, e.body.signature, bodyData);
